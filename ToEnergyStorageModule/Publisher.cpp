...
ACE_TMAIN(int argc, ACE_TCHAR *argv[])
{
  try {
	...
	to_esm_signal.status     = "on";
	to_esm_signal.terminal_voltage = 7.2;
	to_esm_signal.voltage_unit = "kV";
	to_esm_signal.terminal_current = 1.5;
	to_esm_signal.current_unit = "kA";
	to_esm_signal.state_of_charge = 1;
	to_esm_signal.soc_unit = "pu";

    for (int i = 0; i < 10; ++i) {
      DDS::ReturnCode_t error = to_esm_signal_writer->write(to_esm_signal, DDS::HANDLE_NIL);
	  ++to_esm_signal.terminal_voltage;
	  ++to_esm_signal.terminal_current;
